...
def date_facts(text: str) -> str:
    ...
    ("August 3, 2026 is 12 days after July 22, 2026."). The model can use a
    ...
    for i in range(len(found)):
        for j in range(i + 1, len(found)):
            ...
            facts.append(
                (
                    f"{found[j][0]} is {abs(n)} day{'s' if abs(n) != 1 else ''} "
                    f"{'after' if n > 0 else 'before'} {found[i][0]}."
                )
                if n
                else f"{found[j][0]} is the same day as {found[i][0]}."
            )
    return " ".join(facts)


def with_date_facts(state):
    """Append deterministic date facts when enabled in the checkpoint config."""
    ...
    for p, m in zip(probs, meta):
        if m["type"] == "noul":
            ...
            out[m["id"]] = {
                ...
                "score": r2(score),
                "legend": m["legend"],
                "probabilities": {str(i): r2(v) for i, v in enumerate(p)},
